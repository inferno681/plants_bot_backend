...
class MongoPipelineBuilder:
    ...
    @staticmethod
    def build_user_info_pipeline(user_id: PydanticObjectId) -> list:
        return [
            {'$match': {'_id': user_id}},
            {
                '$lookup': {
            ...
            },
            {
                '$project': {
                    '_id': 0,
                    'public_username': 1,
                    'email': {'$arrayElemAt': ['$web.email', 0]},
                    'email_verified': {
                        '$gt': [
                            {'$arrayElemAt': ['$web.email_verified_at', 0]},
                            None,
                        ]
                    },
                    'telegram_id': {
                        '$arrayElemAt': ['$telegram.telegram_id', 0]
                    },
                    'telegram_linked': {'$gt': [{'$size': '$telegram'}, 0]},
                }
            },
        ]
